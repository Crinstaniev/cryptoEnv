...
class MovingAverage(Algorithm):
    def __init__(self, price_feature_pos, short_window=30, long_window=180,
        ...
        self._wallet_usd = np.float64(initial_cap)
        self._wallet_crypto = np.float64(0)
        self._price_feature_pos = price_feature_pos

        # moving average things
        self._short_window = deque([np.float64(0)], maxlen=short_window)
        self._long_window = deque([np.float64(0)], maxlen=long_window)
        self._short_avg_history = [0]
        self._long_avg_history = [0]

    def _update_windows(self, price):
        self._short_window.append(np.float64(price))
        self._long_window.append(np.float64(price))
        self._short_avg_history.append(self._get_short_avg())
        self._long_avg_history.append(self._get_long_avg())

    def _get_long_avg(self):
        return np.array(self._long_window, dtype=np.float64).mean()

    def _get_short_avg(self):
        return np.array(self._short_window, dtype=np.float64).mean()
    ...
    def take_action(self, observation, info=None):
        price = np.float64(observation['features'][self._price_feature_pos])
        ...
        if signal == 0:  # buy
            action = dict(
                signal=signal,
                value=np.array([crypto_to_buy], dtype=np.float64)
            )
        ...
        if signal == 1:  # sell
            action = dict(
                signal=signal,
                value=np.array([crypto_to_sell], dtype=np.float64)
            )
